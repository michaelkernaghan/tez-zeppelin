"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var __1 = require(".."); // Replace '..' with '@airgap/beacon-sdk'
var client = new __1.DAppClient({ name: 'My Sample DApp' });
client
    .requestBroadcast({
    // a valid, forged and signed transaction
    signedTransaction: ''
})
    .then(function (response) {
    console.log('transaction hash', response.transactionHash);
})
    .catch(function (broadcastError) { return console.log(broadcastError); });
//# sourceMappingURL=broadcast-request.js.map